...
function Profile() {
    ...
    const handleToggleImage = async (postId, id,idlike) => {
        try {
            const headers = {
                Authorization: `Bearer ${token}`
            };
            if (like.some((likeItem) => likeItem.post_id === postId && likeItem.user_id === id)) {
                await axios.delete(`http://localhost:3000/api/like/delete/${idlike}`, { headers });
                setLike((prevLikes) => prevLikes.filter((likeItem) => !(likeItem.post_id === postId && likeItem.user_id === id)));
            } else {
                await axios.post(`http://localhost:3000/api/like/store`, {
                    user_id: id,
                    post_id: postId
                }, { headers });
                setLike((prevLikes) => [...prevLikes, { post_id: postId, user_id: id }]);
            }
            setCurrentImage((prevImage) => (prevImage === "image2" ? "image3" : "image2"));
            fetchData();
        } catch (error) {
    ...
    const fetchData = async () => {
        try {
            ...
            const response3 = await axios.get(`http://localhost:3000/api/like/${id}`, {headers});
                    ...
                    <div className="flex">
                        <div className="flex mx-auto">
                            <div className="w-20 h-20 rounded-full">
                                <img src={ urlUser + user.photo } className="rounded-full aspect-square"/>
                            </div>
                            <a className="ml-4 text-2xl font-bold font-sans">{user.nama}</a>
                        </div>
                    </div>
